...
class Model(object):
    def __init__(self, config):
        ...
        self.initializers = initializers.xavier_initializer()

        # 申请占位符
        self.word_inputs = tf.placeholder(dtype=tf.int32, shape=[None, None], name='WordInputs')
        self.seg_inputs = tf.placeholder(dtype=tf.int32, shape=[None, None], name='SegInput')
        ...
        with tf.variable_scope('optimizer'):
            ...
            capped_grad_vars = [[tf.clip_by_value(g, -self.config['clip'],
                                                  self.config['clip']), v] for g, v in grad_vars]
    ...
    def embedding_layer(self, word_inputs, seg_inputs, config, name=None):
        ...
        with tf.variable_scope('word_embedding' if not name else name), tf.device('/cpu:0'):
            ...
            if config['seg_dim']:
                with tf.variable_scope('sge_embedding'), tf.device('/cpu:0'):
    ...
    def crf_loss_layer(self, project_logits, lenghts, name=None):
        ...
        with tf.variable_scope('crf_loss' if not name else name):
            ...
            pad_logits = tf.cast(
                small_value * tf.ones(shape=[self.batch_size, self.num_setps, 1]),
                dtype=tf.float32
            )
            logits = tf.concat(
            ...
            targets = tf.concat(
                [tf.cast(
                    self.num_tags * tf.ones(shape=[self.batch_size, 1]),
                    tf.int32
                ),
                    self.targets],
                axis=-1
    ...
    def evaluate(self, sess, data_manager, id_to_tag):
        ...
        for batch in data_manager.iter_batch():
            ...
            for i in range(len(strings)):
                ...
                golds = data_utils.bioes_to_bio([id_to_tag[int(x)] for x in tags[i][:lengths[i]]])
                preds = data_utils.bioes_to_bio([id_to_tag[int(x)] for x in batch_paths[i][:lengths[i]]])
                for char, glod, pred in zip(string, golds, preds):
                    result.append(' '.join([char, glod, pred]))
    ...
    def evaluate_line(self, sess, inputs, id_to_tag):
        ...
        bath_paths = self.decode(scores, lengths, trans)
        tags = [
            id_to_tag[idx] for idx in bath_paths[0]
        ]
