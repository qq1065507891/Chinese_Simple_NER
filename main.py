...
# 训练相关的
flags.DEFINE_boolean('train', False, '是否采用训练模式')
...
flags.DEFINE_float('dropout', 0.5, 'Dropout size')
flags.DEFINE_integer('batch_size', 120, 'batch_size')
flags.DEFINE_float('lr', 1e-5, 'learning_rete')
flags.DEFINE_string('optimizer', 'adam', '优化器')
flags.DEFINE_boolean('pre_emb', True, '是否使用预训练模型')

flags.DEFINE_integer('max_epoch', 100, '最大轮训次数')
flags.DEFINE_integer('steps_check', 100, 'steps per check')
flags.DEFINE_string('ckpt_path', os.path.join('modelfile', 'ckpt'), '模型保存位置')
flags.DEFINE_string('log_file', 'train.log', '日志保存路径')
...
flags.DEFINE_string('emb_file', os.path.join('data', 'wiki_100.utf8'), '字向量位置')
flags.DEFINE_string('train_file', os.path.join('data', 'ner.train'), '训练数据存放路径')
flags.DEFINE_string('test_file', os.path.join('data', 'ner.test'), '测试数据存放路径')
flags.DEFINE_string('dev_file', os.path.join('data', 'ner.dev'), '验证数据存放路径')

FLAGS = tf.app.flags.FLAGS
assert FLAGS.clip < 5.1, '梯度裁剪不能过大'
assert 0 < FLAGS.dropout < 1, 'dropout必须在0-1之间'
assert FLAGS.lr > 0, 'lr 必须大于0'
assert FLAGS.optimizer in ['adam', 'sgd', 'adagrad'], '优化器必须在adam, sgd, adagrad之间'
...
def train():
    ...
    if not os.path.isfile(FLAGS.map_file):
        if FLAGS.pre_emb:
            ...
            dico_words, word_to_id, id_to_word = data_utils.augment_with_pertrained(
    ...
    print('train_data_num %i, test_data_num %i, dev_data_num %i' % (len(train_data), len(test_data), len(dev_data)))
    ...
    with tf.Session(config=tf_config) as sess:
        model = model_utils.create(sess, Model, FLAGS.ckpt_path, load_word2vec, config, id_to_word, logger, FLAGS.train)
        logger.info('开始训练')
        loss = []
        for i in range(100):
            for batch in train_manager.iter_batch(shuffle=True):
                ...
                if step % FLAGS.steps_check == 0:
                    ...
                    logger.info('iteration:{} step{}/{}, NER LOSS:{:>9.6f}'.format(iterstion,
